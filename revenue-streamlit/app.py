...
def main() -> None:
    ...
    uploaded = st.sidebar.file_uploader(
        "Upload CSV (columns: year,revenue,expenditure)", type=["csv", "xlsx"]
    )

    # load dataframe from upload or default CSV; show a friendly message if default missing
    if uploaded is not None:
        ...
        try:
            ...
            st.warning("Default data file not found: `data/revenue_data.csv`. Showing sample data.")
            df = pd.DataFrame(
                {
                    "year": list(range(2015, 2025)),
                    "revenue": [1000 * (1 + i * 0.1) for i in range(10)],
                    "expenditure": [800 * (1 + i * 0.08) for i in range(10)],
                }
            )
...
if __name__ == "__main__":
    # Running `python app.py` will call main(); preferred method is `streamlit run app.py`.
    main()
